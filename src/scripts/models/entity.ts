...
//   fileInput.click();
// };
// (window as any).onFileSelected = async (event: Event) => {
//   const target = event.target as HTMLInputElement;
//   const files = target.files;

//   // If no files were selected, stop here
//   if (!files || files.length === 0) return;

//   // 1. Map each selected file into a "Promise" that reads the data
...
//         resolve(newFile); // Signal that this specific file is done
//       };
...
//   // 2. Wait for ALL files to finish converting to Base64
//   const processedFiles = await Promise.all(filePromises);

//   // 3. Add all the new files into your current folder array
//   currentFolder.files.push(...processedFiles);

//   // 4. Save to storage ONCE, and update the UI ONCE
//   saveToStorage(rootFolder);
//   navigateToFolder(currentFolder, true);

//   // 5. Reset the input so the exact same files can be uploaded again if needed
//   target.value = '';
// };
